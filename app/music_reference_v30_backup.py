...
def analyze_reference(instruction, artist=None, song=None):
    ...
    if not artist:
        return {'ok':False,'error':'No pude identificar el artista o referencia. Di «analiza el estilo de Quevedo» o «analiza la canción Columbia de Quevedo».'}
    focus=f' canción "{song}"' if song else ' varias canciones representativas'
    q=f'{artist}{focus} BPM tempo género ritmo groove instrumentación producción estructura'
    web=google_web_search(q, 'Busca información pública y fiable. Prioriza metadatos de la canción/artista, BPM/tempo, género y descripciones de producción. No descargues audio, letras, samples ni grabaciones.')
    if not web.get('ok'):
